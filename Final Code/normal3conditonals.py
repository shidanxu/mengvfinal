# This code illustrates the switchpoint concept in pymc. Namely, given duration as 
# a normal distribution, find out where the earlier mean switches to later mean
...
observed_numberOccurrences = pickle.load( open( "../datasets/numberOccurrence.p", "rb" ) )

# print observed_numberOccurrences
# print "\n"
# print type(observed_numberOccurrences)

dataframe = pd.read_pickle("../datasets/training_50000.pickle")
dataframe.loc[:, 'apple'] = (dataframe['device'].isin(['I-phone/I-pad', 'iPhone', 'iPad', 'Mac', 'iPod'])).astype(int)
dataframe.loc[:, 'android'] = (dataframe['device'] == 'android').astype(int)
dataframe.loc[:, 'duration'] =  (dataframe.timeEnd-dataframe.timeStart).astype('timedelta64[s]')
dataframe.loc[:, 'timeStartBucket'] = dataframe.timeStart.apply(lambda x: x.hour)
dataframe = dataframe.loc[dataframe['duration'] >= 0, :]
dataframe = dataframe.loc[dataframe.duration < dataframe.duration.quantile(.995)]

apple_observed = np.array(dataframe.apple)
android_observed = np.array(dataframe.android)
duration_observed = np.array(dataframe.duration)
timeStartBucket = np.array(dataframe.timeStartBucket)
timeStartBucket_corrected = (timeStartBucket + 19 ) %24

# observed_ipFrequencies = pickle.load( open( "ipBinaryFrequency.p", "rb" ) )

# weekend_timeStart_observed = pickle.load(open("../datasets/weekendStartShort.p", "rb"))
# pickle.dump(weekend_timeStart_observed[:100000], open("miniweekendStart.p", "wb"))
# weekday_timeStart_observed = pickle.load(open("../datasets/weekdayStartShort.p", "rb"))
